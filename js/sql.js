...
exports.checkConnection = function(){//проверка подключения
	...
	connection.connect(function(err) {
		if(err) {
			...
			logToFile(info_text, err.message);
		} else {
...
exports.killConnection = function(){//отключение
	connection.end(function(err) {
		if(err) {
			...
			logToFile("Ошибка отключения от БД.", err.message);
		} else {
			consoleLog("Подключение разорвано.");
			logToFile("Подключение разорвано.");
		};
	});
};
...
exports.autentification = function(login, password) {
	return new Promise(function(resolve_end, reject_end) {
		...
		logToFile('Попытка входа в учётную запись{ login : "'+login+'", password : "'+password+'" }');
...
auterisation_inner = async function(login) {
	...
	if(session_obj[0][0] == undefined) {//если сессии нет, то её надо создать
		...
		connection.query("INSERT INTO sessions(SUserKey, SessionId) values(?, ?)", array, function(err, results) {
			...
			logToFile('Ошибка работы с БД, table sessions', err);
		});
		...
		connection.query("UPDATE sessions SET SessionId=? WHERE SUserKey=?", array, function(err, results) {
			if(err) {
				consoleLog("Ошибка обновления ключа сессии в БД", err);
				logToFile('Ошибка работы с БД, table sessions', err);
			};
		});
		//а тут передать это пользователю
...
registration_inner = async function(login, password) {
	var finded_login;
	var finded_login_promise = await connection.promise().query("SELECT * FROM users WHERE Login=?", login, function(err, results) {
		resolve(results);
	}).then(function(value){
		finded_login = value;
	});
	if(finded_login[0][0] == undefined) {
		var date = new Date();
		var year = date.getFullYear();
		var month = date.getMonth();
		var day = date.getDate();
		var time = date.toLocaleTimeString();
		var reg_date_get = year + "-" + month + "-" + day + " " + time;
		var array = [login, password, reg_date_get.toString()];
		connection.query("INSERT INTO users(Login, Passwod, RegDate) values(?, ?, ?)", array, function(err, results) {
			if(err) {
				consoleLog("Ошибка добавления пользвателя.", err);
				logToFile("Ошибка добавления пользвателя.", err);
			} else {
				logToFile("Создан новый пользователь, подробности в БД.");
			};
		});
		return true;
	} else {
		logToFile("Попытка зарегестрировать уже существующую учётную запись {login: "+login+ "}");
		return false;
	};
